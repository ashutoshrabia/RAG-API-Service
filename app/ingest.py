...
def get_clip_model():
    if not hasattr(get_clip_model, "clip_model"):
        get_clip_model.clip_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32").to("cpu")
        get_clip_model.clip_processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
    return get_clip_model.clip_model, get_clip_model.clip_processor


def get_blip_model():
    if not hasattr(get_blip_model, "blip_model"):
        get_blip_model.blip_processor = BlipProcessor.from_pretrained("Salesforce/blip-image-captioning-base")
        get_blip_model.blip_model = BlipForConditionalGeneration.from_pretrained("Salesforce/blip-image-captioning-base").to("cpu")
    return get_blip_model.blip_model, get_blip_model.blip_processor
...
def process_image_file(path, clip_model, clip_processor):
    blip_model, blip_proc = get_blip_model()

    img = Image.open(path).convert("RGB")

    
    blip_inputs = blip_proc(images=img, return_tensors="pt")
    blip_output = blip_model.generate(**blip_inputs, max_new_tokens=50)
    caption = blip_proc.decode(blip_output[0], skip_special_tokens=True)

    clip_inputs = clip_processor(images=img, return_tensors="pt")
    embedding   = clip_model.get_image_features(**clip_inputs).detach().numpy()
